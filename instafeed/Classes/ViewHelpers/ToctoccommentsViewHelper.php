<?php
namespace Cerebrum\Instafeed\ViewHelpers;
...
class ToctoccommentsViewHelper extends \TYPO3\CMS\Fluid\Core\ViewHelper\AbstractViewHelper {
	...
	public function initializeArguments() {
		$this->registerArgument('rawPicture', '\Cerebrum\Instafeed\Domain\Model\RawPicture','The email address to resolve the gravatar for', TRUE);
	}
		
	public function render() {
		$rawPicture=$this->arguments['rawPicture'];
		...
		//return var_dump($conftc);
		$code = $lib->main($content, $conftc, 'tx_instafeed_frontfeeds', $rawPicture->getUid());
		unset($lib);
		return $code;
	}
}